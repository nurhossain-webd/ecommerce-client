...
import { BarChart, ChartCard, DonutChart } from "@/components/admin/charts";
import { ORDER_STATUSES, type OrderStatus } from "@/lib/types";

type Dashboard = { users: number; products: number; categories: number; orders: number; lowStock: number; recentOrders: Order[]; orderStatuses: Record<OrderStatus, number> };
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([usersApi.list(), productsApi.list(), categoriesApi.list(), ordersApi.listAll()])
      .then(([users, products, categories, orders]) => { if (!cancelled) setData({ users: users.length, products: products.length, categories: categories.length, orders: orders.length, lowStock: products.filter((product) => product.stock <= LOW_STOCK_THRESHOLD).length, recentOrders: orders.slice(0, 5), orderStatuses: Object.fromEntries(ORDER_STATUSES.map((status) => [status, orders.filter((order) => order.status === status).length])) as Record<OrderStatus, number> }); })

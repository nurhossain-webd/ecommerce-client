...
export default function AdminCategoriesPage() {
  ...
  const load = useCallback(() => categoriesApi.list().then(setCategories).catch((caught) => setError(getErrorMessage(caught))).finally(() => setLoading(false)), []);
  useEffect(() => { load(); }, [load]);
  const create = async (event: FormEvent) => {
    event.preventDefault(); setError(""); setMessage("");
    try { await categoriesApi.create({ name, status }); setName(""); setStatus("ACTIVE"); setMessage("Category created."); await load(); }
  ...
  const update = async (category: Category) => {
    ...
    try { await categoriesApi.update(category.id, { name: nextName, status: nextStatus }); setMessage("Category updated."); await load(); }
    catch (caught) { setError(getErrorMessage(caught)); }
  };
  const remove = async (category: Category) => {
    if (!window.confirm(`Delete ${category.name}?`)) return;
    try { await categoriesApi.remove(category.id); setCategories((current) => current.filter((item) => item.id !== category.id)); setMessage("Category deleted."); }
    catch (caught) { setError(getErrorMessage(caught)); }
  };

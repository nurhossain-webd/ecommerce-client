...
export default function CartPage() {
  ...
  const checkout = async () => {
    ...
    if (user.role !== "USER") { setError("Checkout is available to customer accounts."); return; }
    if (items.length === 0) return;
    setCheckingOut(true);
    try {
      await ordersApi.create({ items: items.map((item) => ({ productId: item.product.id, quantity: item.quantity })) });
      clearCart();
      router.push("/orders");
  ...
  return <>
    ...
    {items.length === 0 ? <EmptyState icon="cart" title="Your cart is empty" action={<ButtonLink href="/products">Shop products</ButtonLink>}>Add a few products and they will appear here.</EmptyState> : <div className="grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="card p-5 lg:sticky lg:top-40 sm:p-6"><p className="eyebrow">Order summary</p><h2 className="mt-2 text-xl font-semibold text-ink">Estimated total</h2><div className="mt-6 space-y-4 border-y border-line py-5 text-sm"><div className="flex justify-between gap-4 text-muted"><span>Items ({itemCount})</span><span>{formatCurrency(estimatedSubtotal)}</span></div><div className="flex justify-between gap-4 text-muted"><span>Shipping</span><span>Calculated later</span></div></div><div className="flex justify-between gap-4 py-5"><span className="font-semibold text-ink">Estimated subtotal</span><strong className="text-xl text-ink">{formatCurrency(estimatedSubtotal)}</strong></div><p className="mb-5 text-xs leading-5 text-muted">Displayed prices are estimates. Current pricing and stock are verified by the server when you checkout.</p><Button className="w-full" disabled={checkingOut} onClick={checkout}>{checkingOut ? "Placing order..." : user ? "Checkout" : "Log in to checkout"}</Button></aside>
    </div>}
  </>;
}
